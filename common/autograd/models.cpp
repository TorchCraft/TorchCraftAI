...
void LearnableDotProduct::reset() {
  lin_ =
      add(MLP()
              .nIn(2 * nFeatsIn_)
              .nOut(1)
...
ag::Variant LearnableDotProduct::forward(ag::Variant x) {
  auto U = x[0];
  auto V = x[1];
  auto bsz = U.size(0);
  auto n = U.size(1);
  auto m = V.size(1);
  ASSERT_SIZE(U, {bsz, n, nFeatsIn_});
  ASSERT_SIZE(V, {bsz, m, nFeatsIn_});

  U = U.unsqueeze(2)
          .expand({bsz, n, m, nFeatsIn_})
          .contiguous()
          .view({bsz, n * m, nFeatsIn_});
  V = V.unsqueeze(1)
          .expand({bsz, n, m, nFeatsIn_})
          .contiguous()
          .view({bsz, n * m, nFeatsIn_});

  return lin_->forward(torch::cat({U, V}, 2))[0].view({bsz, n, m});
}
...
void MHAttention::reset() {
  ...
  if (dropoutRate_ > 0) {
    dropout_ = add(ag::Dropout().rate(dropoutRate_).make(), "dropout_");
  }

  if (attention_ == Attention::MLP) {
    dotProd_ = add(
        LearnableDotProduct().nFeatsIn(hidDim_).nHid(hidDim_).nLayers(3).make(),
        "dotProd_");
  }
}
...
ag::Variant MHAttention::forward(ag::Variant x) {
  ...
  Q = qLinear_->forward(Q)[0]
          .reshape({bsz, qsz, nHeads_, hidDim_})
          .transpose(1, 2);
  K = kLinear_->forward(K)[0]
          .reshape({bsz, ksz, nHeads_, hidDim_})
          .transpose(1, 2);
  V = vLinear_->forward(V)[0]
          .reshape({bsz, ksz, nHeads_, hidDim_})
          .transpose(1, 2);

  torch::Tensor matmul;
  if (attention_ == Attention::DotProduct) {
    matmul = torch::matmul(Q, K.transpose(-1, -2)).div_(std::sqrt(queryDim_));
  } else if (attention_ == Attention::MLP) {
    matmul = dotProd_
                 ->forward({Q.contiguous().view({-1, qsz, hidDim_}),
                            K.contiguous().view({-1, ksz, hidDim_})})[0]
                 .view({bsz, nHeads_, qsz, ksz});
  }

  ASSERT_SIZE(matmul, {bsz, nHeads_, qsz, ksz});

  if (mask.defined()) {
    float value = softmax_ ? std::numeric_limits<float>::lowest() : 0;
    matmul.masked_fill_((1 - mask).unsqueeze(1), value);
  }
  if (softmax_) {
    matmul = matmul.softmax(-1);
  }

  auto attended = torch::matmul(matmul, V).transpose(1, 2);
  ASSERT_SIZE(attended, {bsz, qsz, nHeads_, hidDim_});

  if (dropout_) {
    attended = dropout_->forward(attended)[0];
  }
