...
#define ALLREDUCE(FType, DType)                                         \
  ...
  template <>                                                           \
  Work Context::allreduce(std::vector<FType>& v, ReduceOp op) {         \
    auto tensor = torch::from_blob(v.data(), v.size(), DType);          \
    return this->allreduce(tensor, op);                                 \
  }
...
#define BROADCAST(FType, DType)                                      \
  ...
  template <>                                                        \
  Work Context::broadcast(std::vector<FType>& v, int root) {         \
    auto tensor = torch::from_blob(v.data(), v.size(), DType);       \
    return this->broadcast(tensor, root);                            \
  }
