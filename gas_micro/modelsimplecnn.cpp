...
#define PRINTTT(tensor) printTensor(#tensor, tensor)
  ...
  auto moveEmbedding = [&]() {
    auto acc = positionsFriendlyCPU.accessor<int, 2>();
...
std::vector<PFMicroActionModel::PFMicroAction> SimpleCNNModel::decodeOutput(
  ...
  LOGGG(moveSelected);
  auto attackSelectedAccessor = attackSelected.accessor<long, 1>();
  auto moveSelectedAccessor = moveSelected.accessor<long, 1>();
  auto actionAccessor = actions.accessor<long, 1>();
