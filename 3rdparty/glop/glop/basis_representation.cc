...
EtaFactorization::~EtaFactorization() {
  Clear();
}

void EtaFactorization::Clear() {
  gtl::STLDeleteElements(&eta_matrix_);
}

void EtaFactorization::Update(
...
bool BasisFactorization::IsRefactorized() const {
  return num_updates_ == 0;
}
...
Status BasisFactorization::ForceRefactorization() {
  ...
  deterministic_time_ += kLuComplexityFactor *
...
void BasisFactorization::LeftSolveForUnitRow(ColIndex j, ScatteredRow* y)
    const {
  SCOPED_TIME_STAT(&stats_);
  RETURN_IF_NULL(y);
  ...
  if (left_pool_mapping_[j] == kInvalidCol) {
    ...
    storage_.ColumnCopyToClearedDenseColumnWithNonZeros(
        left_pool_mapping_[j], x, nz);
  }
...
bool BasisFactorization::IsIdentityBasis() const {
  ...
  for (RowIndex row(0); row < num_rows; ++row) {
    ...
    if (matrix_.column(col).num_entries().value() != 1)
      return false;
