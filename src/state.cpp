...
State::State(std::shared_ptr<tc::Client> client, const StateConfig& config)
...
void State::update() {
  ...
  if (map_ == nullptr) {
    ...
    try {
      ...
      if (!map_->FindBasesForStartingLocations()) {
        for (auto& playerUnits : tcstate_->frame->units) {
          for (auto& unit : playerUnits.second) {
            if (unit.resources > 0) {
              VLOG(0) << "Failed to find BWEM bases for starting locations on "
                      << mapName() << ": \"" << mapTitle() << "\"";
              break;
            }
          }
      ...
      VLOG(1) << "Analysis done, found " << map_->Areas().size()
              << " areas and " << map_->ChokePointCount() << " choke points in "
              << std::chrono::duration_cast<std::chrono::milliseconds>(duration)
                     .count()
              << "ms";
    } catch (const std::exception& exception) {
      VLOG(0) << "Exception running BWEM analysis on " << mapName() << ": \""
              << mapTitle() << "\"";
      VLOG(0) << exception.what();
      throw exception;
    }
  }
...
void State::updateTechnologyStatus() {
  for (auto& tech2status : tech2StatusMap_) {
    ...
    tech2status.second = tcstate_->hasResearched(*tt);
  }
}
...
void State::findEnemyInfo() {
  ...
  constexpr PlayerId kInvalidId = -64;
  PlayerId firstOpponentId = kInvalidId;
  try {
    firstOpponentId = firstOpponent(!config_.guaranteeEnemy);
  } catch (...) {
  }
  ...
  for (auto& it : tcstate_->player_info) {
    ...
    // enemies
    if (pinfo.is_enemy || pinfo.id == firstOpponentId ||
...
PlayerId State::firstOpponent(bool strict) const {
  for (auto& [_, player] : tcstate_->player_info) {
    if (player.is_enemy) {
      VLOG(3) << "Found opponent with strict criteria: ID #" << player.id;
      return player.id;
    }
  }
  if (!strict) {
    for (auto& [_, player] : tcstate_->player_info) {
      auto id = player.id;
      if (id >= 0 && id != playerId()) {
        VLOG(3) << "Found opponent with loose criteria: ID #" << id;
        return id;
      }
    }
  }
