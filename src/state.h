...
class State {
 public:
  explicit State(
      std::shared_ptr<tc::Client> client,
  ...
  ///
  /// @param strict Only consider players flagged by BWAPI as enemies;
  /// don't fall back to non-neutral players that BWAPI thinks might be
  /// observers.
  PlayerId firstOpponent(bool strict = false) const;
  ...
  PlayerId firstToLeave() const {
    return firstToLeave_;
  }
