...
/**
 * Extracts plain features from the current state, in buildtile resolution.
 * Information on the walk-tile level will be average-pooled.
 * boundingBoxBT defaults to all available data, but can also be larger to have
 * constant-size features irrespective of actual map size, for example.
 * NOTE: Only a subset of the feature types offer buildtile featurization.
 * You'll be greeted with a friendly run-time error if support is missing.
 * NOTE: boundingBoxBT is specified in buildtile resolution.
 */
FeatureData featurizePlainBT(
    State* state,
    std::vector<PlainFeatureType> types,
    Rect boundingBoxBT = Rect());
