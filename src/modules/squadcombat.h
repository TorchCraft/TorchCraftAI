...
namespace squadcombat {
template <class T>
void insertBefore(BehaviorList& behaviors, std::shared_ptr<Behavior> behavior) {
  auto matching =
      std::find_if(behaviors.begin(), behaviors.end(), [](auto& behavior) {
        return std::dynamic_pointer_cast<T>(behavior) != nullptr;
      });
  if (matching == behaviors.end()) {
    throw std::runtime_error("Failed to find a matching behavior");
  }
  behaviors.insert(matching, behavior);
}

template <class T>
void removeAll(BehaviorList& behaviors) {
  behaviors.erase(
      std::remove_if(
          behaviors.begin(),
          behaviors.end(),
