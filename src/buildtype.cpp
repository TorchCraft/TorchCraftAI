...
DEFINE_double(buildtype_value_minerals, 1.0, "Value ratio of minerals");
DEFINE_double(buildtype_value_gas, 1.3, "Value ratio of gas");
DEFINE_double(buildtype_value_supply, 6.25, "Value ratio of supply");
DEFINE_double(buildtype_value_terran, 1.0, "Value ratio of Terran units");
DEFINE_double(buildtype_value_protoss, 1.0, "Value ratio of Protoss units");
DEFINE_double(buildtype_value_zerg, 1.2, "Value ratio of Zerg units");
...
const BuildType* getBuildType(BWAPI::UnitType type) {
  ...
  r->sightRange = tc::BW::XYWalktilesPerBuildtile * type.sightRange() / 32;
  ...
  r->subjectiveValue = FLAGS_buildtype_value_minerals * r->mineralCost;
  r->subjectiveValue += FLAGS_buildtype_value_gas * r->gasCost;
  r->subjectiveValue *= r->isTwoUnitsInOneEgg ? 0.5 : 1.0;
  r->subjectiveValue += FLAGS_buildtype_value_supply * r->supplyRequired;
  r->subjectiveValue *= r->isTerran() ? FLAGS_buildtype_value_terran : 1;
  r->subjectiveValue *= r->isProtoss() ? FLAGS_buildtype_value_protoss : 1;
  r->subjectiveValue *= r->isZerg() ? FLAGS_buildtype_value_zerg : 1;
...
const BuildType* Special_Map_Revealer;
...
void initialize() {
  ...
  Special_Map_Revealer = getUnitBuildType(101);
