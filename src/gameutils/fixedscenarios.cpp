...
FixedScenario& asymmetric(
  ...
  for (const UnitCount& unit : unitsAlly) {
    scenario.allies().emplace_back(SpawnPosition{unit.count,
                                                 unit.unitType,
                                                 centerX - separationX,
                                                 centerY - separationY,
                                                 unitSpread,
  ...
  for (const UnitCount& unit : unitsEnemy) {
    scenario.enemies().emplace_back(SpawnPosition{unit.count,
                                                  unit.unitType,
                                                  centerX + separationX,
                                                  centerY + separationY,
                                                  unitSpread,
...
FixedScenarioGroup simpleScenarios() {
  ...
  hugMiddleEasy.allies().emplace_back(SpawnPosition{
      1, bt::Terran_Vulture, mapMidpointX, mapMidpointY, 15.0, 15.0});
  hugMiddleEasy.enemies().emplace_back(SpawnPosition{
      1, bt::Zerg_Overlord, mapMidpointX, mapMidpointY, 0.0, 0.0});
  ...
  ignoreCivilians.allies().emplace_back(SpawnPosition{
      1, bt::Zerg_Zergling, mapMidpointX, mapMidpointY, 0.0, 12.0});
  ignoreCivilians.enemies().emplace_back(SpawnPosition{
      4, bt::Terran_Civilian, mapMidpointX, mapMidpointY, 0.0, 12.0});
  ignoreCivilians.enemies().emplace_back(SpawnPosition{
      1, bt::Protoss_High_Templar, mapMidpointX, mapMidpointY, 0.0, 12.0});
...
FixedScenarioGroup regroupingScenarios() {
  ...
  auto makeSurround = [&](std::string name, bt unit) {
    ...
    for (int i = 0; i < unitCount; ++i) {
      ...
      scenario.allies().emplace_back(
          SpawnPosition{1, unit, (int)allyX, (int)allyY});
    }
    auto enemyR =
    ...
    scenario.enemies().emplace_back(
        SpawnPosition{unitCount, unit, (int)enemyX, (int)enemyY});
  };
  auto makeConga = [&](std::string name, bt unit) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      scenario.allies().emplace_back(SpawnPosition{1, unit, allyX, allyY});
      scenario.enemies().emplace_back(SpawnPosition{1, unit, enemyX, enemyY});
    }
  };
...
FixedScenarioGroup kitingScenarios() {
  ...
  for (int i = 0; i < 5; ++i) {
    vulturesZealots.allies().emplace_back(
        SpawnPosition{1, bt::Terran_Vulture, 140, 120 + 2 * i, 5.0, 5.0});
  }
  for (int i = 0; i < 10; ++i) {
    vulturesZealots.enemies().emplace_back(
        SpawnPosition{1, bt::Protoss_Zealot, 120, 120 + 2 * i, 5.0, 5.0});
  }
  auto& vultureZealot = group.add("vu_zl");
  {
    ...
    vultureZealot.allies().emplace_back(SpawnPosition{
        vultures, bt::Terran_Vulture, center + ctrx, center + ctry, 8.0, 8.0});
    vultureZealot.enemies().emplace_back(SpawnPosition{
        zealots, bt::Protoss_Zealot, center - ctrx, center - ctry, 8.0, 8.0});
  }
  ...
  for (int i = 0; i < 6; ++i) {
    marinesZealots.allies().emplace_back(
        SpawnPosition{1, bt::Terran_Marine, 140, 120 + i * 2, 5.0, 5.0});
  }
  for (int i = 0; i < 4; ++i) {
    marinesZealots.enemies().emplace_back(
        SpawnPosition{1, bt::Protoss_Zealot, 120, 130 + i * 2, 5.0, 5.0});
  }
...
FixedScenarioGroup miscellaneousScenarios() {
  ...
  for (int i = 0; i < 10; ++i) {
    mutalisk10Corsair5.allies().emplace_back(
        SpawnPosition{1, bt::Zerg_Mutalisk, 140 + i * 2, 110, 5.0, 5.0});
  }
  for (int i = 0; i < 5; ++i) {
    mutalisk10Corsair5.enemies().emplace_back(
        SpawnPosition{1, bt::Protoss_Corsair, 110, 120 + i * 3, 5.0, 5.0});
  }

  auto& mutalisk8Corsair5 = group.add("8mu_5co");
  for (int i = 0; i < 8; ++i) {
    mutalisk8Corsair5.allies().emplace_back(
        SpawnPosition{1, bt::Zerg_Mutalisk, 140 + i * 2, 110, 5.0, 5.0});
  }
  for (int i = 0; i < 5; ++i) {
    mutalisk8Corsair5.enemies().emplace_back(
        SpawnPosition{1, bt::Protoss_Corsair, 110, 120 + i * 3, 5.0, 5.0});
  }

  // Hydralisks vs. dragoons on high ground - need to go up ramp
  auto& hydralisksDragoonsRamp = group.add("3hy_2dr");
  hydralisksDragoonsRamp.allies().emplace_back(
      SpawnPosition{3, bt::Zerg_Hydralisk, 145, 145, 3.0, 3.0});
  hydralisksDragoonsRamp.enemies().emplace_back(
      SpawnPosition{2, bt::Protoss_Dragoon, 125, 128, 3.0, 3.0});
  ...
  hydralisksTank.allies().emplace_back(
      SpawnPosition{2, bt::Zerg_Hydralisk, 140, 140, 5.0, 5.0});
  hydralisksTank.enemies().emplace_back(
      SpawnPosition{1, bt::Terran_Siege_Tank_Siege_Mode, 125, 110, 5.0, 5.0});

  // Tank min range scenario - note siege-mode adds friendly fire
  auto& hydralisksTanks = group.add("4hy_2sst");
  hydralisksTanks.allies().emplace_back(
      SpawnPosition{4, bt::Zerg_Hydralisk, 90, 120, 8.0, 8.0});
  hydralisksTanks.enemies().emplace_back(
      SpawnPosition{1, bt::Terran_Siege_Tank_Siege_Mode, 155, 110, 5.0, 5.0});
  hydralisksTanks.enemies().emplace_back(
      SpawnPosition{1, bt::Terran_Siege_Tank_Siege_Mode, 165, 140, 5.0, 5.0});

  // Zerglings must surround zealots instead of attacking asap
  auto& zerglingsZealots = group.add("30zg_10zl");
  for (int i = 0; i < 30; ++i) {
    zerglingsZealots.allies().emplace_back(
        SpawnPosition{1, bt::Zerg_Zergling, 130 + i, 130, 3.0, 3.0});
  }
  for (int i = 0; i < 10; ++i) {
    zerglingsZealots.enemies().emplace_back(
        SpawnPosition{1, bt::Protoss_Zealot, 110, 120 + i * 2, 3.0, 3.0});
  }
  ...
  auto addSpreadGroup =
    ...
    for (auto& v : globalMult) {
      v = common::Rand::sample(
          std::uniform_real_distribution<float>(1 / varUnits, varUnits));
    }
    std::array<int, 2> xCenter{
        mapMidpointX * 3 / 2,
        mapMidpointX / 2,
    };
    if (rand() % 2) {
      // make p1 and p2 completely equivalent
      std::swap(xCenter[0], xCenter[1]);
    }
    for (auto u : counts) {
      std::array<int, 2> playerCounts = {
  ...
  addSpreadGroup(
      ...
      2.0f,
      {
          {bt::Zerg_Zergling, rangeOn(5, 20)},
          {bt::Zerg_Mutalisk, rangeOn(5, 20)},
          {bt::Zerg_Hydralisk, rangeOn(5, 20)},
          {bt::Zerg_Scourge, rangeOn(5, 20)},
      });
  addSpreadGroup(
      "spread_zvz_ground",
      2.0f,
      {
          {bt::Zerg_Zergling, rangeOn(5, 20)},
          // Include Hydras in 20% of the scenarios
          {bt::Zerg_Hydralisk, int(rangeOn(0, 100) < 20) * rangeOn(2, 4)},
      });
  addSpreadGroup(
      "spread_zg",
      2.0f,
...
FixedScenarioGroup defilerScenarios() {
  ...
  auto defilerShuffle = [](FixedScenario& scenario,
                           const std::vector<UnitCount>& unitsAlly,
                           const std::vector<UnitCount>& unitsEnemy) {
