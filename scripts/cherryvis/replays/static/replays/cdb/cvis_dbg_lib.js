...
function cvis_dbg_lib_init(global_data, cvis_state) {
  ...
  function parse_cvis_links(html) {
    ...
    html.find('a.cvis-link-position').click(function(e) {
      ...
      var rect = get_walk_tile_rect(parseInt($(this).attr('data-pos-x-wt')), parseInt($(this).attr('data-pos-y-wt')));
      cvis_state.lib.highlighted_rectangles = [rect];
      update_highlights();
    ...
    html.find('a.cvis-link-frame').each(function() {
      ...
      e.preventDefault();
      _cvis_set_current_frame($(this).attr('data-frame-id'));
    });
  ...
  function load_cvis_json_file(filename, success_fn, fail_fn) {
    $.get(
      "get/cvis/?cvis=" + encodeURIComponent(cvis_state.cvis_path) + "&f=" + encodeURIComponent(filename),
      g => success_fn(g),
      "json"
    ).fail(fail_fn);
  }
  ...
  */
  ...
  cvis_state.functions.openbw_set_current_frame = _cvis_set_current_frame;
  cvis_state.lib = {
    highlighted_units: [],
