...
parser.add_argument(
    '-p', '--partition', type=str, default='scavenge', help='slurm partition')
parser.add_argument('-r', '--reservation', type=str, help='slurm reservation')
parser.add_argument(
    '-n', '--num-jobs', type=int, default=1000, help='Number of jobs')
parser.add_argument(
    '-j',
    '--num-parallel-jobs',
    type=int,
    default=1000,
    help='Max number of jobs running at once')
parser.add_argument('-J', '--job-name', type=str, default='bos-rlv3-samples')
args = parser.parse_args()

array = [''] * args.num_jobs

binary = '/workspace/build/scripts/bo-switch/bos-train'
cmd = [binary]
cmd += '-mode online -v -1 -vmodule bos-train-supervised-online=1'.split()
cmd += '-bos_model_type idle'.split()
# XXX hack for using local scratch
cmd += '-playoutput /workspace/playoutput'.split()
cmd += '-num_game_threads 12'.split()
cmd += '-bandit ucb1rolling -strategy training'.split()

job_name = args.job_name
images = workspace.create_snapshot(
    job_name=job_name, workspace=args.workspace, workspace_image=args.image)
...
sbatch_script_path = job.create_sbatch_script_array(
    array=array,
    jobdir=jobdir,
    images=images,
    cmd_args=cmd,
